...
static int              frame_count = 1800;  // Hardcoded 1800 since a 30hz camera can capture upto 1800 frames in a minute 
...
static void process_image(const void *p, int size)
{
	...
	// store start time when first frame is read
        if(framecnt==1)
            start_time=frame_time.tv_sec + (frame_time.tv_nsec/BILLION) ;
        
        dump_ppm(bigbuffer, ((size*6)/4), framecnt, &frame_time);
	
	// Calculate total elapsed time        
	total_time=(frame_time.tv_sec + (frame_time.tv_nsec/BILLION)) - start_time;
	
	// Calculate and store frameRate when elapsed time becomes more than or equal to 1 minute and set the flag to true
        if(total_time >= 60.0000 && tflag==0)
        {

	tflag=1;    
	
	    frameRate=(double)framecnt/total_time;
...
static void mainloop(void)
{
    ...
    while (count > 0)
    {
		...
		// Print Average frame rate when all the frames are dumped		
		if(count==0)
	   	{
			printf("\n");			
			printf("Average Frame Rate over a period of 1 minute: %f\n",frameRate);
		}
                break;
            }
